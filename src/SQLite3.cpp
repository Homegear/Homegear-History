		...
		if(GD::bl->io.fileExists(_databasePath + _databaseFilename))
		{
			...
			else
			{
				if(!_backupPath.empty() && !_backupFilename.empty())
				{
					...
					{
						if(GD::bl->io.fileExists(_backupPath + _backupFilename + std::to_string(GD::settings.databaseMaxBackups() - 1)))
						{
							if(!GD::bl->io.deleteFile(_backupPath + _backupFilename + std::to_string(GD::settings.databaseMaxBackups() - 1)))
							{
